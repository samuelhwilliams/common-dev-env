...
then
   ...
   commands:
      up            configure, build and run all services; will pull updates from services' git repos and rebuild images
      quickup       as per up, but without updating services' git repos or rebuilding images
      halt          stop all containers
      reload        stop all containers, rebuild them, and restart them (including commodity fragments)
      quickreload   as per reload, but without rebuilding images 
      destroy       stop and remove all containers, then remove all built images and (optionally) reset common-dev-env configuration
      repair        set the docker-compose configuration to use *this* dev-env, for users with several common-dev-env instances

   flags:
      -n, --nopull  for \`up\` and \`reload\` only; avoid docker hub ratelimiting by not checking for updates to FROM images used in Dockerfiles"
fi
